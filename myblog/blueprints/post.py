...
def show_post(post_id):
    ...
    with current_app.app_context():
        ...
        if post.private and current_user != post.author:
            current_app.logger.warning(
                f"User {current_user.id} tried peeking post {post.id} which is private, but was bounced back"
            )
            abort(404)
        ...
        return render_template(
            "blog/show_post.html", post=post, comments=comments, is_admin=is_admin()
        )
    ...
    with current_app.app_context():
        ...
        if current_user == post.author or is_admin():
            ...
            if form.validate_on_submit():
                ...
                db.session.commit()
                current_app.logger.info(
                    f"Post {post.id} has been edited by user {current_user.id}"
                )
            ...
            current_app.logger.warning(
                f"User {current_user.id} attempted editing post {post.id}, but was bounced back"
            )
            abort(403)
...
def delete_post(post_id):
    ...
    with current_app.app_context():
        ...
        if current_user == post.author or is_admin():
            ...
            db.session.commit()
            current_app.logger.info(
                f"Post {post.id} has been deleted by user {current_user.id}"
            )
            return redirect(url_for("index.index"))
        else:
            current_app.logger.warning(
                f"User {current_user.id} attempted deleting post {post.id}, but was bounced back"
            )
            abort(403)
